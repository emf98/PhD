...
print("SFS FOR PC MODEL OVER EUROPE, 3 CLASSES.")
print('###########################################')
print("Start 14-day lag file ... Import statements")
...
# load input solvers
infile = open("../../EOF_Analysis/PVsolver_14days.p","rb",)
PVsolver = pickle.load(infile)  ##pv on an isentropic surface, 350
infile.close()

infile = open("../../EOF_Analysis/EHFsolver_14days.p","rb",)
EHFsolver = pickle.load(infile)  ##ZMehf vertical cross section along longitudes
infile.close()

infile = open("../../EOF_Analysis/GPHsolver_14days.p","rb",)
...
## PV
PV_EOF_nw, PV_EOF_nw2d, PV_eigenv, PV_VarEx, PV_PC = EOF_def(PVsolver, 175)
## EHF
...
# load output data
infile = open("../../temperature_data/europetemps_anom_classedTHREE.p","rb",)
...
pickle.dump(sele_ind, open("Europe3_SelectIndices_14days.p", 'wb'))
...
plt.title("Frequency of Selected PCs for 25 CVs of RF Model for Europe (14 Days, 3 Classes)")
sele_ind1 = np.array(sele_ind)
sele_ind1 = sele_ind1.reshape(25*8)
plt.hist(sele_ind1, bins=np.arange(200))
plt.savefig('../images/Europe3_14days_selefeature_hist.png')
